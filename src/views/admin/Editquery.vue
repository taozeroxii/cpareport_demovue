...
export default {
  ...
  created() {
    axios.get(  `http://172.18.2.2:3010/api/admin/findOldquerybyid/${this.$route.params.id}`, { headers: { "x-access-token": localStorage.token } } ).then((res) => {
  ...
  methods: {
    ...
    validate() {
      ...
      if (this.$refs.form.validate() === true) {
        ...
        axios.put( `http://172.18.2.2:3010/api/admin/editquery/${this.$route.params.id}`,this.form, { headers: { "x-access-token": localStorage.token } } ).then(() => {
            this.loadlogData();
            var menu = {
                  menu_title:this.menu_title,
                  sql_head:this.sql_head
            }
            axios.put( `http://172.18.2.2:3010/api/admin/editmenuquery/${this.sql_file}`,menu,{ headers: { "x-access-token": localStorage.token } } ).then(() => {  });
    ...
    loadlogData(){
    axios.get(  `http://172.18.2.2:3010/api/admin/logsqlupdate-list/${this.$route.params.id}`, { headers: { "x-access-token": localStorage.token } } ).then((res) => {
